(function ($, undefined) {
	...
	$.jstree.plugins.actions = function (options, parent) {
		...
		this._create_action = function (node_id, action_id) {
			...
			action_el.className = $(action).attr('class');//  action.class;
		...
		this._set_action = function (node_id, obj, action) {//该方法经过修改，主要增加内部插入和根据types类别定义图标功能
			if (action === null) return;

			var place = obj.querySelector(action.action.selector);
			if(action.action.append){//新增内部插入方法
				var typ = this.get_type(obj);
				if(typ && action.action.types[typ]){
					action.action_el.className=action.action.types[typ].icon || action.action_el.className;
				}
				
				place.appendChild(action.action_el);
			}
			else{

...
<html lang="en">
	...
	<body class="no-skin" oncontextmenu="if(self.event)self.event.returnValue=false;return false;">
		<div class="main-container">
			...
			<div id='logItems' class='fade in' style="position:absolute;background-color: #ffe; z-index:99999;"></div>
		</div>
		...
		jQuery(function($){
			...
			logitemsForumat = function(cellvalue, options, rowObject ){
				...
				return "<table class='table table-bordered table-striped'><thead class='thin-border-bottom'><th>名称</th><th>数值</th></thead><tbody>"+tr+"</tbody></table>";
			},			
			setJqgridData = function(){
				...
				*/
				...
				invockeService('sys.logs.query.service',condition,callback,page,rowNum,records);
				return false;
			};
			$("#myJqGrid").jqGrid({
				...
				gridComplete: function () {
					$(">td:eq(3)",$("#myJqGrid .jqgrow")).mouseover(function (event) {
						...
						if($('#logItems').height() >= border_top){
							top_pos = 0;
						}
						else if(event.pageY + $('#logItems').height() >= border_top){
					    	 if(event.pageY >= $('#logItems').height()){
					        ...
					        top_pos = event.pageY-$('#logItems').height()/2;
					     }
						if(top_pos < 0){
				        	top_pos = 0;
				        }

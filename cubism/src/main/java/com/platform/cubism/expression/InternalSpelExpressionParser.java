...
import com.platform.cubism.expression.ast.Assign;
import com.platform.cubism.expression.ast.BeanReference;
import com.platform.cubism.expression.ast.BooleanLiteral;
import com.platform.cubism.expression.ast.CompoundExpression;
import com.platform.cubism.expression.ast.ConstructorReference;
import com.platform.cubism.expression.ast.Elvis;
import com.platform.cubism.expression.ast.FunctionReference;
import com.platform.cubism.expression.ast.Identifier;
import com.platform.cubism.expression.ast.Indexer;
import com.platform.cubism.expression.ast.InlineList;
import com.platform.cubism.expression.ast.Literal;
import com.platform.cubism.expression.ast.MethodReference;
import com.platform.cubism.expression.ast.NullLiteral;
import com.platform.cubism.expression.ast.OpAnd;
import com.platform.cubism.expression.ast.OpDivide;
import com.platform.cubism.expression.ast.OpEQ;
import com.platform.cubism.expression.ast.OpGE;
import com.platform.cubism.expression.ast.OpGT;
import com.platform.cubism.expression.ast.OpLE;
import com.platform.cubism.expression.ast.OpLT;
import com.platform.cubism.expression.ast.OpMinus;
import com.platform.cubism.expression.ast.OpModulus;
import com.platform.cubism.expression.ast.OpMultiply;
import com.platform.cubism.expression.ast.OpNE;
import com.platform.cubism.expression.ast.OpOr;
import com.platform.cubism.expression.ast.OpPlus;
import com.platform.cubism.expression.ast.OperatorBetween;
import com.platform.cubism.expression.ast.OperatorInstanceof;
import com.platform.cubism.expression.ast.OperatorMatches;
import com.platform.cubism.expression.ast.OperatorNot;
import com.platform.cubism.expression.ast.OperatorPower;
import com.platform.cubism.expression.ast.Projection;
import com.platform.cubism.expression.ast.PropertyOrFieldReference;
import com.platform.cubism.expression.ast.QualifiedIdentifier;
import com.platform.cubism.expression.ast.Selection;
import com.platform.cubism.expression.ast.SpelNodeImpl;
import com.platform.cubism.expression.ast.StringLiteral;
import com.platform.cubism.expression.ast.Ternary;
import com.platform.cubism.expression.ast.TypeReference;
import com.platform.cubism.expression.ast.VariableReference;
...
class InternalSpelExpressionParser extends TemplateAwareExpressionParser {
	...
	private SpelNodeImpl eatExpression() {
		...
		if (moreTokens()) {
			...
			} else if (t.kind == TokenKind.ELVIS) { // a?:b (a if it isn't null,
													// otherwise b)
				if (expr == null) {
	...
	// relationalExpression : sumExpression (relationalOperator^
	// sumExpression)?;
	...
	// unaryExpression: (PLUS^ | MINUS^ | BANG^) unaryExpression |
	// primaryExpression ;
	...
	private void consumeArguments(List<SpelNodeImpl> accumulatedArguments) {
		...
		Token next = null;
		do {
			nextToken();// consume ( (first time through) or comma (subsequent
						// times)
			Token t = peekToken();
			if (t == null) {
	...
	/**
	 * Eat an identifier, possibly qualified (meaning that it is dotted). TODO
	 * AndyC Could create complete identifiers (a.b.c) here rather than a
	 * sequence of them? (a, b, c)
	 */
	private SpelNodeImpl eatPossiblyQualifiedId() {
		...
		return new QualifiedIdentifier(toPos(startnode.startpos, qualifiedIdPieces.get(qualifiedIdPieces.size() - 1).getEndPosition()),
	...
	// Dollars are normally separate tokens but
	// there we want to combine a series of identifiers and dollars into a
	// single identifier
	...
	private boolean peekToken(TokenKind desiredTokenKind, boolean consumeIfMatched) {
		...
		if (t.kind == desiredTokenKind) {
			...
			if (desiredTokenKind == TokenKind.IDENTIFIER) {
				// might be one of the textual forms of the operators (e.g. NE
				// for != ) - in which case we can treat it as an identifier
				// The list is represented here:
				// Tokenizer.alternativeOperatorNames and those ones are in
				// order in the TokenKind enum
				if (t.kind.ordinal() >= TokenKind.DIV.ordinal() && t.kind.ordinal() <= TokenKind.NOT.ordinal() && t.data != null) {
					// if t.data were null, we'd know it wasn't the textual
					// form, it was the symbol form
					return true;
				}
			}
			return false;

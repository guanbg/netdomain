...
@WebServlet(name = "CropRegister", urlPatterns = { "/CropRegister" })
public class CropRegister extends HttpServlet{
	private static final long serialVersionUID = 3647130198292389255L;
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String serviceName = "crop.registeuser.add";
		...
		if(HeadHelper.isSuccess(ret)){
			try{
				...
				in2.addField("tmplname","email.userreg.tmpl");
				in2.addField("title",getValue("email.userreg.title"));
				in2.addField("url","/CropEmailConfirm");
				in2.addField("email",in.getFieldValue("email"));
				in2.addField("company_name",in.getFieldValue("company_name"));
				CStruc parameters = JsonFactory.createStruc("parameters");
				parameters.addField("user_id", ret.getFieldValue("user_id"));
				...
				Json in2 = JsonFactory.create();
				in2.addField("user_id", ret.getFieldValue("user_id"));
				ServiceFactory.executeService("crop.registeuser.delete", in2);
				
				Json err = JsonFactory.create();
				err.addStruc(HeadHelper.createRetHead(serviceName, "1090", "发送邮件错误，请检查电子邮箱是否正确!", MsgLevel.D));
				response.getWriter().print(err.toJson());
				response.flushBuffer();
				return;
			}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user_id == null || user_id.length() <= 0){
			Json err = JsonFactory.create();
			err.addStruc(HeadHelper.createRetHead("CropRegister", "1091", "用户注册信息不正确，不能发送邮件!", MsgLevel.D));
			response.getWriter().print(err.toJson());
			response.flushBuffer();
			return;
		}
		...
		try{
			...
			CStruc parameters = JsonFactory.createStruc("parameters");
			parameters.addField("user_id", user_id);
			in2.addStruc(parameters);
			Json ret2 = new SendMail().execute(in2);
			if(HeadHelper.isFailed(ret2)){
				ret.getObject().setName(null);

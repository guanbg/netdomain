...
import static com.platform.cubism.base.JsonFactory.createArray;
...
@WebServlet(name = "FileUploadDB", urlPatterns = { "*.sys_dbfile_upload" })
public class UploadDB extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(parts.isEmpty()){
			...
			writeResponse(HeadHelper.createRetHead("sys.upload.db.files", "10001", "保存文件时失败：没有上传文件内容", "D", "F"), response);
			return;
		}
		else{
			...
			try {
				...
				for (Part part : parts) {
					...
					files.add(createStruc("files",new CField[]{
						...
						createField("diskfilename",diskFilename),
				...
				writeResponse(HeadHelper.createRetHead("sys.upload.db.files", "10009", "保存文件时失败："+t.getMessage(), "D", "F"), response);
			}
			
			if(files == null || files.isEmpty()){
				logger.error("文件上传错误：没有上传文件内容，请重新上传");
				writeResponse(HeadHelper.createRetHead("sys.upload.db.files", "10008", "保存文件时失败：没有上传文件内容", "D", "F"), response);
				return;
			}
			
			try{
				Json in =JsonFactory.create("in");
				CArray ca = createArray("files");
				for(CStruc cs : files.getRecords()){
					ca.add(createStruc("files",new CField[]{
						createField("file_id",getUUID()),
						createField("file_name",cs.getFieldValue("filename")),
						createField("file_identify",cs.getFieldValue("fileidentify")),
						createField("file_type",cs.getFieldValue("filetype")),
						createField("file_path",cs.getFieldValue("filepath")),
						createField("file_size", cs.getFieldValue("filesize"))
					}));
				}
				in.addArray(ca);				
				checkSysHead(in, request);
				Json ret = ServiceFactory.executeService("busi.fms_files.add", in);
				
				if(HeadHelper.isFailed(ret)){
					for(CStruc s : files.getRecords()){
						delFile(s.getFieldValue("diskfilename"));
					}
				...
				writeResponse(HeadHelper.createRetHead("busi.fms_files.add", "10009", "写数据库时失败，"+t.getMessage(), "D", "F"), response);
			}
		}
	}

 ...
 * interface, configuring the plain old JDBC {@link java.sql.DriverManager} via
 * bean properties, and returning a new {@link java.sql.Connection} from every
 ...
 * conjunction with a simple JNDI environment. Pool-assuming
 * <code>Connection.close()</code> calls will simply close the Connection, so
 * any DataSource-aware persistence code should work.
 * 
 * <p>
 * <b>NOTE: Within special class loading environments such as OSGi, this class
 * is effectively superseded by {@link SimpleDriverDataSource} due to general
 * class loading issues with the JDBC DriverManager that be resolved through
 * direct Driver usage (which is exactly what SimpleDriverDataSource does).</b>
 * 
 * <p>
 ...
 * switching to and from a local DataSource bean like this class. For tests, you
 ...
 * the bean definition to a local DataSource (which is simpler and thus
 * recommended).
 * 
 * <p>
...
public class DriverManagerDataSource extends AbstractDriverBasedDataSource {
	...
	public DriverManagerDataSource(Properties conProps) {
		Assert.notNull(conProps);
		String url = conProps.getProperty("url");
		// String username = conProps.getProperty("user");
	 ...
	 * <i>pre-registered</i> JDBC drivers.</b> If you need to register a new
	 * driver, consider using {@link SimpleDriverDataSource} instead.
	 * Alternatively, consider initializing the JDBC driver yourself before
	 * instantiating this DataSource. The "driverClassName" property is mainly
	 * preserved for backwards compatibility, as well as for migrating between
	 * Commons DBCP and this DataSource.
	 * 
	...
	public void setDriverClassName(String driverClassName) {
		...
		try {
			Class.forName(driverClassNameToUse, true, CubismHelper.getDefaultClassLoader());

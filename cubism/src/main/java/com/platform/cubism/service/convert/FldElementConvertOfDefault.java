...
public class FldElementConvertOfDefault implements FldElementConvert {
	private static Pattern fieldPattern = Pattern.compile("^in\\..*[^\\.].+");
	...
	public boolean isConvert(FldElement fldElem, CStruc data) {
		...
		if (fieldPattern.matcher(value).matches()) {// in.xxx
			return true;
		}
		return false;
	}
